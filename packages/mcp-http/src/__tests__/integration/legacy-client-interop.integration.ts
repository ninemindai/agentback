...
// Real 2025-era clients, pinned. Every other test in this package drives the
...
//
// Each alias is a released `@modelcontextprotocol/sdk` 1.x. They predate the v2
// package split entirely and vendor their own zod 3, so they share nothing with
// our tree but the wire — which is the point.
//
// Three, not one, because "the legacy era" is not one revision: these span
// `2025-03-26` through `2025-06-18` as their newest supported version, and they
// bracket the range we claim to serve. One client version is a smoke test; the
// endpoints of a range are interop. This is removal criterion 1 in §5.1 of
// docs/proposals/mcp-2026-stateless.md, which gates deleting the session path.
import {Client as C111} from 'mcp-client-1-11/client/index.js';
import {StreamableHTTPClientTransport as T111} from 'mcp-client-1-11/client/streamableHttp.js';
import {LATEST_PROTOCOL_VERSION as V111} from 'mcp-client-1-11/types.js';
import {Client as C117} from 'mcp-client-1-17/client/index.js';
import {StreamableHTTPClientTransport as T117} from 'mcp-client-1-17/client/streamableHttp.js';
import {LATEST_PROTOCOL_VERSION as V117} from 'mcp-client-1-17/types.js';
import {Client as C129} from 'mcp-client-1-29/client/index.js';
import {StreamableHTTPClientTransport as T129} from 'mcp-client-1-29/client/streamableHttp.js';
import {LATEST_PROTOCOL_VERSION as V129} from 'mcp-client-1-29/types.js';

// Three SDK versions have three structurally different `Client` types, so they
// do not form a constructable union. Narrowing to the surface this test
// actually drives is the fix — and it doubles as documentation of how small
// that surface is: connect, enumerate, call, close. Interop is not a wide API
// question.
interface AnyClient {
...
type ClientCtor = new (info: {name: string; version: string}) => AnyClient;
type TransportCtor = new (
  url: URL,
  opts?: {requestInit?: {headers: Record<string, string>}},
) => {sessionId?: string};

/**
 * The matrix. Each entry earns its slot:
 * - 1.11.0 — the first 1.x with the Streamable HTTP transport at all.
 * - 1.17.0 — mid-range, a year before the 2026-07-28 revision.
 * - 1.29.0 — the last release before the v2 package split.
 */
const CLIENTS = [
  {label: '1.11.0', Client: C111, Transport: T111, latest: V111},
  {label: '1.17.0', Client: C117, Transport: T117, latest: V117},
  {label: '1.29.0', Client: C129, Transport: T129, latest: V129},
] as unknown as {
  label: string;
  Client: ClientCtor;
...
describe.each(CLIENTS)(
  'interop with released 2025-era MCP client $label',
  ({
    label,
    Client: LegacyClient,
    ...
    async function connectLegacy(token?: string) {
      const client = new LegacyClient({name: 'legacy-client', version: label});
    ...
    it('is pinned to a client that cannot speak the 2026 revision', () => {
      // 2026-07-28 is the revision under test; anything a 1.x client claims as
      // its newest is by definition older. Asserting the shape rather than one
      // literal keeps the guard working as the matrix grows.
      expect(LEGACY_LATEST).toMatch(/^2025-/);
    });
