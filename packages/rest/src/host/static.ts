...
/**
 * Return a prefix handler suitable for {@link RestServer.addFetchPrefix} that
 * reads static files from `dir` on the local filesystem.
 *
...
export function serveStaticDir(
  dir: string,
): (suffix: string) => Promise<globalThis.Response | undefined> {
