 ...
 * we support on the Web path: `true`/`undefined` reflect the request origin
 * (or `*` when absent), `string` is a fixed origin, `string[]`/`RegExp`/
 * `RegExp[]` allow-list, and a `false` denies (returns `undefined`).
 *
...
function resolveOrigin(
  ...
  // Function-form origin callbacks are not supported on the Web path; treat as
  // reflect (the permissive default) rather than silently denying.
...
export function createCorsWebMiddleware(
  ...
  const middleware: WebMiddleware = async (req, _ctx, next) => {
    if (req.method === 'OPTIONS') {
      ...
      applyCorsHeaders(headers, opts, req);
