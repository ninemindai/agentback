...
import {Binding, Context} from '@agentback/context';

import {types} from 'util';
import {CoreTags, service} from '../../index.js';
import {asService} from '../../service.js';
...
describe('@service', () => {
  ...
  it('injects a service instance matching a sub class', async () => {
    ...
    const controller = await ctx.get<MyController>('controllers.MyController');
    expect(controller.myService).toBeInstanceOf(MySubService);
  });
  ...
  it('allows asProxyWithInterceptors flag', async () => {
    ...
    const controller = await ctx.get<MyController>('controllers.MyController');
    expect(types.isProxy(controller.myService)).toBe(true);
  });
  ...
  it('reports error if serviceInterface not found - symbol', async () => {
    ...
    return expect(
      ctx.get<MyController>('controllers.MyController'),
    ).rejects.toThrow(/No binding found for Symbol\(MyService\)/);
  });
