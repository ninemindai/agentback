...
import {DecoratorFactory} from '@agentback/metadata';
import assert from 'assert';
import {loggers} from '@agentback/common';
import {Context} from './context.js';
import {invokeMethodWithInterceptors} from './interceptor.js';
import {ResolutionSession} from './resolution-session.js';
import {resolveInjectedArguments} from './resolver.js';
import {transformValueOrPromise, ValueOrPromise} from './value-promise.js';

const log = loggers('loopback:context:invocation');
...
function invokeTargetMethodWithInjection(
  ...
  if (log.debug.enabled) {
    log.debug('Invoking method %s', methodName);
    if (nonInjectedArgs?.length) {
      log.debug('Non-injected arguments:', nonInjectedArgs);
    }
  }
  ...
  return transformValueOrPromise(argsOrPromise, args => {
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Injected arguments for %s:', methodName, args);
    }
...
function invokeTargetMethod(
  ...
  /* istanbul ignore if */
  if (log.debug.enabled) {
    log.debug('Invoking method %s', getTargetName(target, methodName), args);
  }
  // Invoke the target method
  const result = targetWithMethods[methodName](...args);
  /* istanbul ignore if */
  if (log.debug.enabled) {
    log.debug('Method invoked: %s', getTargetName(target, methodName), result);
  }
  return result;
}
