...
import {Constructor} from './value-promise.js';

const log = loggers('loopback:context:binding-inspector');
...
export function bindingTemplateFor<T>(
  ...
  log.debug('class %s has binding metadata', cls.name, spec);
...
export function createBindingFromClass<T>(
  ...
  log.debug('create binding from class %s with options', cls.name, options);
  try {
