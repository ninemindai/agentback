...
import {isBindingAddress} from './binding-filter.js';
...
const log = loggers('loopback:context:resolver');
...
export function instantiateClass<T extends object>(
  ...
  if (log.debug.enabled) {
    log.debug('Instantiating %s', getTargetName(ctor));
  ...
  const inst: ValueOrPromise<T> = transformValueOrPromise(
    ...
    args => {
      /* istanbul ignore if */
      if (log.debug.enabled) {
        log.debug('Injected arguments for %s():', ctor.name, args);
      }
  ...
  return transformValueOrPromise(propertiesOrPromise, props => {
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Injected properties for %s:', ctor.name, props);
    }
...
export function resolveInjectedArguments(
  ...
  if (log.debug.enabled) {
    log.debug(
      'Resolving injected arguments for %s',
      getTargetName(target, method),
    );
  }
...
export function resolveInjectedProperties(
  ...
  if (log.debug.enabled) {
    log.debug(
      'Resolving injected properties for %s',
      getTargetName(constructor),
    );
  }
