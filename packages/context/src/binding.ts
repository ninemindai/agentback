...
import {inspectInjections} from './inject.js';
import {createProxyWithInterceptors} from './interception-proxy.js';
import {invokeMethod} from './invocation.js';
import {JSONObject} from './json-types.js';
import {ContextTags} from './keys.js';
...
import {instantiateClass} from './resolver.js';
import {
  BoundValue,
  Constructor,
  isPromiseLike,
...
export class Binding<T = BoundValue> extends EventEmitter {
  ...
  getValue(
    ...
    if (log.debug.enabled) {
      log.debug('Get value for binding %s', this.key);
    }
  ...
  to(value: T): this {
    ...
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Bind %s to constant:', this.key, value);
    }
    this._source = {
      type: BindingType.CONSTANT,
      value,
  ...
  toDynamicValue(
    ...
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Bind %s to dynamic value:', this.key, factory);
    }
    this._source = {
      type: BindingType.DYNAMIC_VALUE,
      value: factory,
  ...
  toProvider(providerClass: Constructor<Provider<T>>): this {
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Bind %s to provider %s', this.key, providerClass.name);
    }
    this._source = {
      type: BindingType.PROVIDER,
      value: providerClass,
  ...
  toClass<C extends T & object>(ctor: Constructor<C>): this {
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Bind %s to class %s', this.key, ctor.name);
    }
    this._source = {
      type: BindingType.CLASS,
      value: ctor,
  ...
  toAlias(keyWithPath: BindingAddress<T>) {
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Bind %s to alias %s', this.key, keyWithPath);
    }
    this._source = {
      type: BindingType.ALIAS,
      value: keyWithPath,
