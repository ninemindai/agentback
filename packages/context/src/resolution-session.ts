...
import {BindingSelector} from './binding-filter.js';
import {Context} from './context.js';
import {Injection, InjectionMetadata} from './inject.js';
import {BoundValue, tryWithFinally, ValueOrPromise} from './value-promise.js';

const log = loggers('loopback:context:resolver:session');
...
export class ResolutionSession {
  ...
  pushInjection(injection: Readonly<Injection>) {
    ...
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Resolution path:', this.getResolutionPath());
    }
  }
  ...
  popInjection() {
    ...
    if (log.debug.enabled) {
      ...
      log.debug('Resolution path:', this.getResolutionPath() || '<empty>');
    }
    return injection;
  }
  ...
  pushBinding(binding: Readonly<Binding>) {
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Enter binding:', binding.toJSON());
    }
  ...
  popBinding(): Readonly<Binding> {
    ...
    /* istanbul ignore if */
    if (log.debug.enabled) {
      log.debug('Exit binding:', binding?.toJSON());
      log.debug('Resolution path:', this.getResolutionPath() || '<empty>');
    }
    return binding;
  }
