...
import {describe, it, beforeEach, describe as context, expect} from 'vitest';

import {Debugger} from '@agentback/common';
...
describe('Context constructor', () => {
  ...
  it('adds subclass name as the prefix', () => {
    ...
    expect(ctx.name).toMatch(
      new RegExp(`^TestContext-${UNIQUE_ID_PATTERN.source}$`),
    );
  });
...
describe('Context', () => {
  ...
  describe('close()', () => {
    ...
    it('removes listeners from parent context', () => {
      ...
      expect(childCtx.observers!.size).toEqual(1);
      expect(childCtx.parentEventListener).toBeInstanceOf(Function);
  ...
  describe('debug', () => {
    ...
    it('sets up debug for subclasses with the class name', () => {
      ...
      const myCtx = new MyContext();
      expect(myCtx.debugFn.namespace).toEqual(
        'loopback:context:mycontext:debug',
      );
    });
    ...
    it('allows debug namespace for subclasses', () => {
      ...
      const myCtx = new MyContext();
      expect(myCtx.debugFn.namespace).toEqual('myapp:my-context:debug');
    });
  });
