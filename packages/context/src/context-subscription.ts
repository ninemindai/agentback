...
import {pEventIterator as iterator, pEventMultiple as multiple} from 'p-event';
...
const log = loggers('loopback:context:subscription');
...
export class ContextSubscriptionManager extends EventEmitter {
  ...
  private _debug(...args: unknown[]) {
    /* istanbul ignore if */
    if (!log.debug.enabled) return;
    const formatter = args.shift();
    if (typeof formatter === 'string') {
      log.debug(`[%s] ${formatter}`, this.context.name, ...args);
    } else {
      log.debug('[%s] ', this.context.name, formatter, ...args);
    }
  }
  ...
  async waitUntilPendingNotificationsDone(timeout?: number) {
    ...
    log.debug('Number of pending notifications: %d', count);
    if (count === 0) return;
