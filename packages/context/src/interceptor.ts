...
import {Binding, BindingTemplate} from './binding.js';
import {injectable} from './binding-decorator.js';
import {
...
import {sortBindingsByPhase} from './binding-sorter.js';
...
import {Constructor, tryWithFinally, ValueOrPromise} from './value-promise.js';
const log = loggers('loopback:context:interceptor');
...
export class InterceptedInvocationContext extends InvocationContext {
  ...
  getGlobalInterceptorBindingKeys(): string[] {
    ...
    log.debug('Global interceptor binding keys:', keys);
    return keys;
  }
  ...
  loadInterceptors() {
    ...
    log.debug('Interceptors for %s', this.targetName, interceptors);
    return interceptors;
  }
}
