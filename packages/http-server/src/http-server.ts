...
const log = loggers('loopback:http-server');
...
export class HttpServer {
  ...
  constructor(
    ...
    log.debug('Http server options', serverOptions);
    ...
    if (this.serverOptions.path) {
      log.debug('Http server with IPC path %s', this.serverOptions.path);
   ...
   */
  public async start() {
    log.debug('Starting http server', this.serverOptions);
    ...
    log.debug('Http server is listening on', this.url);
  }
  ...
  public async stop() {
    ...
    if (this._stoppable != null) {
      log.debug('Stopping http server with graceful close');
