...
import {HttpOptions, HttpServer} from '../../index.js';
...
describe('HttpServer (unit)', () => {
  ...
  describe('constructor', () => {
    ...
    it('applies keepAliveTimeout option', () => {
      ...
      expect(server.server.keepAliveTimeout).toBe(5000);
    });
    ...
    it('applies headersTimeout option', () => {
      ...
      expect(server.server.headersTimeout).toBe(6000);
    });
    ...
    it('applies maxConnections option', () => {
      ...
      expect(server.server.maxConnections).toBe(100);
    });
    ...
    it('applies maxHeadersCount option', () => {
      ...
      expect(server.server.maxHeadersCount).toBe(50);
    });
    ...
    it('applies timeout option', () => {
      ...
      expect(server.server.timeout).toBe(10000);
    });
    ...
    it('applies multiple server properties', () => {
      ...
      expect(server.server.keepAliveTimeout).toBe(1000);
      expect(server.server.headersTimeout).toBe(2000);
      expect(server.server.maxConnections).toBe(10);
      expect(server.server.maxHeadersCount).toBe(20);
      expect(server.server.timeout).toBe(3000);
    });
    ...
    it('sets up graceful stop with gracePeriodForClose', () => {
      ...
      expect(server.serverOptions.gracePeriodForClose).toBe(5000);
    });
    ...
    it('handles gracePeriodForClose set to 0', () => {
      ...
      expect(server.serverOptions.gracePeriodForClose).toBe(0);
    });

    it('does not set up stoppable when gracePeriodForClose is undefined', () => {
      server = new HttpServer(dummyRequestHandler, {});
      expect(server.serverOptions.gracePeriodForClose).toBeUndefined();
    });
  });
  ...
  describe('server options', () => {
    it('merges provided options with defaults', () => {
      ...
      expect(server.serverOptions).toMatchObject(options);
    });
    ...
    it('preserves all provided options', () => {
      ...
      expect(server.serverOptions.port).toBe(3000);
      expect(server.serverOptions.host).toBe('localhost');
      expect(server.serverOptions.keepAliveTimeout).toBe(5000);
      expect(server.serverOptions.headersTimeout).toBe(6000);
    });
  });
  ...
  describe('server instance', () => {
    it('exposes underlying http.Server instance', () => {
      server = new HttpServer(dummyRequestHandler);
      expect(server.server).toHaveProperty('listen');
      expect(server.server).toHaveProperty('close');
    });
  });
