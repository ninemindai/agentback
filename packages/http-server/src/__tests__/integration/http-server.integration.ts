...
import {HttpOptions, HttpServer, HttpsOptions} from '../../index.js';
...
describe('HttpServer (integration)', () => {
  ...
  skipOnTravis(it, 'formats IPv6 url correctly', async () => {
    ...
    await server.start();
    expect([6, 'IPv6']).toContain(getAddressFamily(server));
  ...
  it('stops server', async () => {
    ...
    await server.start();
    await server.stop();
    await expect(httpGetAsync(server.url)).rejects.toThrow(/ECONNREFUSED/);
  });
  ...
  it('stops server with shorter grace period and inflight request', async () => {
    ...
    const socketPromise = expect(req).rejects.toThrow(/socket hang up/);
    // Stop the server
    await server.stop();
    // No more new connections are accepted
    await expect(httpGetAsync(server.url)).rejects.toThrow(/ECONNREFUSED/);
  ...
  it('applies server properties', async () => {
    ...
    expect(server.server).toHaveProperty('keepAliveTimeout', 101);
    expect(server.server).toHaveProperty('headersTimeout', 102);
    expect(server.server).toHaveProperty('maxConnections', 103);
    expect(server.server).toHaveProperty('maxHeadersCount', 104);
    expect(server.server).toHaveProperty('timeout', 105);
  });

  it('exports original port', async () => {
    server = new HttpServer(dummyRequestHandler, {port: 0});
    expect(server).toHaveProperty('port', 0);
  });
  ...
  it('exports original host', async () => {
    server = new HttpServer(dummyRequestHandler);
    expect(server).toHaveProperty('host', undefined);
  });
  ...
  it('exports protocol', async () => {
    ...
    expect(server.protocol).toMatch(/http|https/);
  });
  ...
  it('exports url', async () => {
    ...
    expect(server.url).toMatch(/http|https:\/\//);
  });
  ...
  it('reports error when the server cannot be started', async () => {
    ...
    await expect(anotherServer.start()).rejects.toThrow(/EADDRINUSE/);
  });

  it('supports HTTP over IPv4', async () => {
    server = new HttpServer(dummyRequestHandler, {host: '127.0.0.1'});
    await server.start();
    expect([4, 'IPv4']).toContain(getAddressFamily(server));
    const response = await httpGetAsync(server.url);
    expect(response.statusCode).toBe(200);
  });
  ...
  skipOnTravis(it, 'handles IPv6 loopback address in HTTPS', async () => {
    ...
    await httpsServer.start();
    expect([6, 'IPv6']).toContain(getAddressFamily(httpsServer));
    const response = await httpsGetAsync(httpsServer.url);
    expect(response.statusCode).toBe(200);
  });
  ...
  it('converts host from [::] to [::1] in url', async () => {
    ...
    await server.start();
    expect(server.url).toBe(`http://[::1]:${server.port}`);
  });
  ...
  it('supports HTTP over unix socket', async () => {
    ...
    await server.start();
    expect(getAddressFamily(server)).toBe('ipc');
    expect(server.address).toEqual(socketPath);
    expect(server.host).toBeUndefined();
    expect(server.port).toEqual(0);
    expect(server.url).toEqual('http+unix://' + encodeURIComponent(socketPath));
    await supertest(server.url).get('/').expect(200);
  });
  ...
  it('supports HTTP over Windows named pipe', async () => {
    ...
    await server.start();
    expect(getAddressFamily(server)).toBe('ipc');
    expect(server.url).toEqual(namedPipe);
  });
  ...
  it('rejects invalid named pipe on Windows', async () => {
    ...
    }).toThrow(/Named pipe test\.pipe does NOT start with/);
  });
