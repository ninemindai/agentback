...
describe('runDeploy', () => {
  ...
  it('deploys, parses url, verifies', async () => {
    ...
    const out = await runDeploy(parseDeployArgs(['vercel']), vercelTarget, {
      exec,
      fetchFn: okFetch,
      cwd,
    });
  ...
  it('throws an actionable error when not authed', async () => {
    ...
    await expect(
      runDeploy(parseDeployArgs(['vercel']), vercelTarget, {
        exec,
        fetchFn: okFetch,
        cwd,
      }),
  ...
  it('refuses to clobber an existing api/index.ts without --force', async () => {
    ...
    await expect(
      runDeploy(parseDeployArgs(['vercel', '--eject']), vercelTarget, {
        exec: fakeExec({}),
        fetchFn: okFetch,
        cwd,
      }),
