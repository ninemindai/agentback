...
/**
 * A registered client application (e.g. an API-key or OAuth2 client) whose
 * access can be governed by scope policy. `allowedScopes`/`disallowedScopes`
 * are consumed by `@agentback/authorization`'s scope governance.
 */
export interface ClientApplication extends Principal {
  /** Human-readable name. */
  name?: string;
  /**
   * Scopes this application may request. Omit (or include the `ALL` sentinel)
   * to permit any scope. The `INTERNAL` sentinel must be listed explicitly —
   * `ALL` does not grant it.
   */
  allowedScopes?: string[];
  /** Scopes explicitly denied; takes precedence over `allowedScopes`. */
