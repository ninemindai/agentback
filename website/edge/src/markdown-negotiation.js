...
// agentback.dev served from Cloudflare Workers Static Assets, with markdown
// content negotiation.
//
// The build (website/build.mjs → website/dist) emits an authored `.md` twin
// next to every page plus an `index.md` for the homepage. Cloudflare uploads
// `dist` as the Worker's static assets (the `ASSETS` binding); this Worker
// serves them and, when an agent sends `Accept: text/markdown`, returns the
// authored markdown twin instead of the HTML — a *local* asset lookup, no
// origin and no cross-origin fetch (the previous GitHub-Pages-proxy design did
// one round-trip back to GitHub per markdown request; this does not).
//
