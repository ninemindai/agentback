...
ESM/Zod/MCP fork of LoopBack 4 — a slim modern subset of `@loopback/core` + REST for building HTTP and MCP services out of the same DI container. ESM-only, Node 22.13+, TypeScript 7.0, pnpm 11 workspaces. Alpha (v0.7.1 published to npm — all `@agentback/*` packages + the `create-agentback` scaffolder); API still settling. Scaffold a new app with `npm create agentback my-service [--template rest|mcp|hybrid]`.
...
3. **New capability packages** (no upstream LB4 ancestor — added on top of the ported core). Each has a README; read it before touching the package:
   ...
   - `mcp-http` — exposes the in-process MCP server over the MCP **Streamable HTTP** transport, mounted on the REST app's Express. **`protocol: 'stateless'`** (opt-in, both hosts) swaps the session machinery for the SDK's `createMcpHandler`, which serves the **2026-07-28** revision _and_ 2025-era traffic from one endpoint — no `Mcp-Session-Id`, one server per request, scales behind a plain round-robin LB. Under it `perSession` becomes per-**request** discovery (its DI context is released when the SDK closes that request's server, after any streamed progress), so wrap expensive binders in **`cachedPerPrincipal`** — the binder now runs on every request. `eventStore`/resumable SSE and GET/DELETE are session features and do not apply. The stdio counterpart is `@agentback/mcp`'s `protocol: 'both'` (`serveStdio`, era pinned per connection). Default stays 2025-era on both; see [docs/proposals/mcp-2026-stateless.md](docs/proposals/mcp-2026-stateless.md). Kept separate so `mcp` stays Express-free. **(This is the HTTP transport the old "not yet implemented" note referred to — it now exists.)**
