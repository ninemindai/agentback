...
**Status:** v1 SHIPPED (2026-07-06) — implemented as designed after the DX + eng reviews below; deviations: no `AgentsComponent` (installAgent binds everything; a component with nothing to contribute is dead weight) and no v1 config schema (the enforced-knobs rule cut `maxConcurrentSessions`/`perTurnTimeout` until a turn-execution wrapper enforces them). The spike phase was folded into the package's own integration tests (mock model, no network) — every phase-1 proof criterion is a CI test.
