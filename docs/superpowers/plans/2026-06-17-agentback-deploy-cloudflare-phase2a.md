...
function writeEdit(
  ...
  if (existsSync(abs) && !force) {
    ...
    if (
      !cur.includes('Generated by `agentback deploy`') &&
...
export async function runDeploy(
  ...
  enforceConsoleGate({
    ...
    unsafePublicConsole: args.unsafePublicConsole,
  });
  ...
  for (const edit of target.generateConfig(opts))
    writeEdit(deps.cwd, edit, true);
  ...
  const verify = await verifyDeploy(
    url,
    {verifyPath: args.verifyPath || target.defaultVerifyPath()},
    deps.fetchFn,
  );
...
- [ ] **Step 5: Rename + update the test** — `run-vercel.unit.ts` → `run-deploy.unit.ts`. Replace `runVercelDeploy(args, {...})` calls with `runDeploy(args, vercelTarget, {...})` (import `vercelTarget` from `../../targets/vercel.js`). Update the marker assertions to the generic `Generated by \`agentback deploy\``. No assertion's _meaning_ changes.
...
- Produces:
  ...
  ```ts
  // generate-worker.ts
  export function generateWorker(b: {
    entry: string;
    exportName: string;
  }): string;
  // merge-wrangler.ts
...
export const cloudflareTarget: DeployTarget = {
  ...
  async deploy(args: DeployArgs, deps: RunDeps): Promise<{url: string}> {
    ...
    const res = await deps.exec('wrangler', [
      'deploy',
      ...(args.prod ? [] : ['--env', 'preview']),
    ]);
    if (res.code !== 0) {
...
export function scanImports(modules: string[]): Diagnostic {
  for (const m of modules) {
    ...
    if (denied) {
      const hint =
        denied.includes('fs') || denied.includes('path')
