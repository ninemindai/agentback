...
Two small edges worth knowing. A timeout marker's `seatKeyId` is always the `''` sentinel, even when the seat has a real key row: the registry stamps the acting key onto the turn context only *after* the command method returns, and a timed-out turn's never did (the marker still identifies the turn by `actor` + `requestId`). And **what the deadline covers differs by runtime**: the in-process runtimes race the whole turn action, state load and `initialState` included, so a deadline can fire before `receive` is ever reached (on the event-sourced runtime that case gets the log line only — there is no stored identity to append a marker to yet), whereas Redis races `receive` alone, so a hanging `initialState` there is **not** deadline-guarded at all — no `TurnTimeoutError`, no marker, and the caller waits indefinitely. The renewal cap still lets that seat's lease lapse, so the identity stays claimable by other callers; it is a known follow-up gap on the caller side. Every marker for a turn that reached `receive` lands on both journaling runtimes.
