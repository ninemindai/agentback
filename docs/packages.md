...
| `@agentback/actors`                | Zod-typed actor runtime port — `@actor`/`@actorCommand`, per-identity serialized turns, idempotent replay, in-memory adapter (**experimental**) |
| `@agentback/actors-redis`          | Redis-backed actor runtime adapter — per-identity leases + atomic state/dedup commit (**experimental**)                                         |
