...
| Actors              | `@actor`/`@actorCommand`/`@actorQuery`, `ACTOR_REGISTRY`, `@injectActor`, `InMemoryActorsComponent`          | Stateful entity at `{type,id}`: serialized turns, idempotent `requestId`, lease-free queries, optional event log; `@agentback/actors`(`-redis`, `-do`)                                                                      |
