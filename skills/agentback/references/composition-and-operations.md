...
**`mountComponent(app, Class, {name, allowOverride})`** mounts a class with no specifier — for a `Component` written at runtime — with identical governance and an `Installed` back. **`PluginBindings.REGISTRY`** answers what is mounted right now (`mounted()` / `componentRefs()`), which a report cannot, and `@agentback/introspection` exposes it as the `plugin` kind so an agent can inspect the tree before changing it. Mounting is a write and is deliberately absent from the read-only introspection surface. **No agent-callable mount tool ships** — wire one yourself behind your own `@authorize` scope and an allow-list of components you already ship; see the recipe in `packages/plugin/README.md`. **Plugins are trusted code**: this package governs collisions, ordering and lifecycle, never capability, and the boundary for untrusted code is a process or container.
